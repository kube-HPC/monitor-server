...
class etcdData extends events {
    ...
    async getJobs() {
        try {
            ...
            let res = await this.etcd3.getSortLimit('/jobResults',["Create", "Descend"]);
            res = Object.entries(res).filter(([key,val])=>key.endsWith('/status')).map(([key,val])=>{
                return { key: key.split('/')[2], ...JSON.parse(val) }
            })
