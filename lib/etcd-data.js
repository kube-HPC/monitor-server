...
class etcdData extends Events {
    ...
    async _getJobStatus(limit = 100) {
        const list = await this._client.jobStatus.list({ order: 'create', sort: 'desc', limit });
        return this._listToMap(list);
    }
    ...
    async _getAlgorithms() {
        const algorithmList = await this._client.algorithms.templatesStore.list();
    ...
    async _getAlgorithmBuilds() {
        const list = await this._client._client.getByQuery('/algorithms/builds/');
        const results = [];
        Object.entries(list).forEach(([, v]) => {
            results.push({ ...v.value, buildId: v.key2 });
        });
        return results;
    }
