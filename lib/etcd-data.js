...
class etcdData extends events {
    ...
    _startInterval() {
        this.intervalId = setInterval(async () => {
            const results = await Promise.all([this.getJobs(), this.getWorkers(), this.getAlgorithms()]);
            this._lastWorkers = results[1];
            this.emit('result', { jobs: results[0], workers: results[1], algorithms: results[2] })
        }, 1000);
    }
    ...
    async getAlgorithms() {
        let algorithms = [];
        try {
            const res = await this.etcd3.get('/templatesStore/');
            algorithms = Object.entries(res).map(([k, v]) => ({ key: k.split('/')[2], data: JSON.parse(v) }));
        }
        catch (error) {
        }
        return algorithms;
