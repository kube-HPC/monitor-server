...
class etcdData extends Events {
    ...
    _startInterval() {
        ...
        this.intervalId = setInterval(async () => {
            ...
            try {
                this._working = true;
                const [results, status, discovery, algorithms] = await Promise.all([this.getJobResults(), this.getJobStatus(), this.getDiscovery(), this.getAlgorithms()]);
                this._lastJobs = status;
                const jobs = [];
                ...
                this.emit('result', { jobs, discovery, algorithms });
            }
            catch (error) {
                log.error(error);
            }
