...
const serverInit = async (options) => {
    ...
    app.delete('/store/algorithms/:algorithmName', async (req, res) => {
        ...
        try {
            ...
            while (apiRes.body.error !== undefined && apiRes.body.error.code === 400) {
                for (let e of apiRes.body.error.details.executions){
                    try {
                        await stopExecution(e)
                    } catch (error) {
                        log.error(`error stopExecution ==> ${error}`);
                    }
                }
                for (pipeline of apiRes.body.error.details.pipelines) {
                    try {
                        await deletePipelineFromStore(pipeline)
                    } catch(error) {
                        log.error(`error deletePipeFromStore ==> ${error}`);
                    }
                }
                apiRes = await deleteAlgorithmFromStore(req.params.algorithmName);
            }
