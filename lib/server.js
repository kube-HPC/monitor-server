...
const { getReadme, postReadme } = require('./api-server')
...
const serverInit = async (options) => {
    ...
    app.get('/algorithms/:name', async (req, res) => {
        ...
        let readme =result.readme? result.readme: '';
            

            res.status(200).json({name:req.params.name,readme});
        } catch (error) {
    ...
    app.post('/algorithms/:name', async (req, res) => {
        try {
