...
const { addAlgorithmForDebug,
    deleteAlgorithmForDebug, addPipeFromSimulator, 
    execStoredPipe, deleteAlgorithmFromStore,
    insertAlgorithmToStore } = require('./api-server')
let log;
...
const serverInit = async (options) => {
    ...
    app.delete('/store/algorithms/:algorithmName', async (req, res) => {
        ...
        const httpCode = "200";
        const data = await deleteAlgorithmFromStore(req.params.algorithmName)
        res.status(httpCode).json({ message: httpCodes[httpCode], data });
    });
