...
app.delete('/store/algorithms/:algorithmName', async (req, res) => {
    ...
    try {
        ...
        if (apiRes.statusCode === 400) {
            ...
            executions.forEach(e => promisesList.push(stopExecution(e)))
            pipelines.forEach(p => promisesList.push(deletePipelineFromStore(p)))

            Promise.all(promisesList).then(async res => {
                console.log(`result ${JSON.stringify(res)}`);
                await deleteAlgorithmFromStore(req.params.algorithmName);
