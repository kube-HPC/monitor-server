...
const { addAlgorithmForDebug } = require('./api-server')
let log;
...
const serverInit = async (options) => {
    ...
    app.post('/webhook/progress', async (req, res) => {
        const httpCode = "200";
        if (req.body && req.body.algorithmName) {
            const res = await addAlgorithmForDebug(req.body.algorithmName)
        }
        //  res.status(httpCode).json({ message: httpCodes[httpCode] });
    })
    ...
    app.delete('/debug/delete', async (req, res) => {
        const httpCode = "200";
        const data = await deleteAlgorithmForDebug(req.body)
        res.status(httpCode).json({ message: httpCodes[httpCode], data });
    })
