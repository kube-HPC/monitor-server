...
class ResultGather extends Events {
    ...
    async _resultsGatherInterval() {
        ...
        try {
            ...
            for (const exp of jobs) {
                exp.jobs = await this.cleanGraph(exp.jobs, taskMap, batchMap, nodeMap, pipelines);
                // exp.jobs = this.cleanLargeObjects(exp.jobs);
            }
    ...
    // cleanLargeObjects(experimentJobs) {
    //     return experimentJobs.map(j => {
    //         const items = ['pipeline', 'userPipeline'];
    //         const flowInputProps = ['flowInput', 'flowInputMetadata'];
    //         for (const itemName of items) {
    //             const item = j[itemName];
    //             if (item) {
    //                 // check each of flowInputProps
    //                 for (const flowInputName of flowInputProps) {
    //                     const flowInputItem = item[flowInputName];
    //                     if (flowInputItem) {
    //                         const size = Buffer.byteLength(JSON.stringify(flowInputItem));
    //                         if (size > this._options.sizes.maxFlowInputSize) {
    //                             item[flowInputName] = { truncated: `Size of object (${size}) is larger than ${this._options.sizes.maxFlowInputSize} ` };
    //                         }
    //                     }
    //                 }
    //             }
