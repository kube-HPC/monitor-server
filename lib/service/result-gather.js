...
class ResultGather extends Events {
    ...
    async _resultsGatherInterval() {
        ...
        try {
            this._working = true;
            const { nodeStatistics, diskUsage } = nodeStatisticsData.getLatestResult();
            ...
            this.emit('result', { nodeStatistics, diskUsage, pipelinesStats, jobs, logs: redisLogs, discovery, algorithms, pipelines, experiments, algorithmBuilds, boards: { batchMap, taskMap, nodeMap } });
        }
        catch (error) {
