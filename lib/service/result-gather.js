...
class ResultGather extends Events {
    ...
    async _resultsGatherInterval() {
        ...
        try {
            ...
            // const jobs = await etcdApi.getJobs('main');
            //   let { discovery, algorithms, algorithmsForDebug, pipelines, algorithmBuilds } = await etcdApi.getResult();
            let { jobs, discovery, algorithms, algorithmsForDebug, pipelines, algorithmBuilds } = etcdApi.getLastResults();
            for (const exp of jobs) {
                exp.jobs = await this.graphGather(exp.jobs)
            }
            ...
            this.emit('result', { nodeStatistics, pipelinesStats, jobs, logs: redisLogs, discovery, algorithms, algorithmsForDebug, pipelines, algorithmBuilds });
        }
        catch (error) {
    ...
    async graphGather(experimentJobs) {
