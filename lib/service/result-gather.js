...
class ResultGather extends Events {
    ...
    _addBoardToNode({ node, job, batchMap, taskMap }) {
        ...
        if (node.batch && node.batch.some(task => (task.metricsPath && task.metricsPath.tensorboard.path))) {
            hasMetrics = true;
            node.boards = { tensorboard: {} };
            node.boards.tensorboard.board = batchMap[job.key] && batchMap[job.key][node.nodeName];
        }
        else {
            if (node.metricsPath && node.metricsPath.tensorboard.path) {
                hasMetrics = true;
                node.boards = { tensorboard: { hasMetrics: true } };
                node.boards.tensorboard.board = taskMap[node.taskId];
            }
        }
        return hasMetrics;
    }
    ...
    async _resultsGatherInterval() {
        ...
        try {
            ...
            for (const exp of jobs) {
                exp.jobs = await this.graphGather(exp.jobs,boards)
            }
