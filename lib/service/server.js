...
const serverInit = async options => {
    ...
    const { datasourceService } = options;
    ...
    app.get('/logs/set', (req, res) => {
        ...
        const { settings, options: logOptions } = logs;
        res.json({ settings, options: logOptions });
    });
    app.get('/logs', async (req, res) => {
        const { taskId, podName, source, nodeKind, logMode, sort } = req.query;
        const data = await logs.getLogs({ taskId, podName, source, nodeKind, logMode, sort });
