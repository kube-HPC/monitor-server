...
const roomRegister = (data, socket) => {
    socket.join(data.name);
    resultGather.experimentRegister(data.name.split('experiment:')[1])
...
const serverInit = async options => {
    ...
    io.on("connection", socket => {
        ...
        socket.on('experiment-register', ({ name, lastRoom }) => {
    ...
    resultGather.on("result", res => {
        res.jobs.forEach(job => {
            io.to(`experiment:${job.experimentName}`).emit(CONSTS.progress, { ...res, jobs: job.jobs })
        }
        )
    ...
    app.get("/logs", async (req, res) => {
        const data = await logs.getLogs(req.query.taskId, req.query.podName, req.query.source);
