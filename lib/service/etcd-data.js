...
class EtcdData extends Events {
    ...
    async _getExecutions(experimentName) {
        const list = await this._client.executions.stored.list({ jobId: `${this._experimentFilter(experimentName)}`, order: 'create', sort: 'desc' });
    ...
    async _getJobResults(experimentName) {
        const list = await this._client.jobs.results.list({ jobId: `${this._experimentFilter(experimentName)}`, order: 'create', sort: 'desc' });
        return this._listToMap(list);
    }

    async getFlowInputByJobId(jobId) {
        const pipeline = await this._client.executions.stored.get({ jobId });
        return pipeline?.flowInput;
    }

    async _getJobStatus(experimentName, limit = 100) {
        const jobId = `${this._experimentFilter(experimentName)}`;
