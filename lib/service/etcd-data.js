...
class etcdData extends Events {
    ...
    async getResult() {
        const [discovery, algorithms, pipelines, algorithmBuilds, experiments] = await Promise.all([
            this._getDiscovery(),
            this._getAlgorithms(),
        ...
        this.lastResults = { discovery, ...algorithms, pipelines, algorithmBuilds, experiments };
    ...
    async _getStoredPipelines(experimentName) {
        const list = await this._client.pipelines.list({ jobId: `${experimentName}:`, order: 'create', sort: 'desc' });
        return list;
    }
    ...
    async _getJobStatus(experimentName, limit = 100) {
        const list = await this._client.jobs.status.list({ jobId: `${experimentName}:`, order: 'create', sort: 'desc', limit });
        return this._listToMap(list);
    }
    ...
    async _getAlgorithms() {
        const algorithms = await this._client.algorithms.store.list();
    ...
    async _getBoards() {
        const list = this._client.tensorboard.list();;
        return list;
    }
