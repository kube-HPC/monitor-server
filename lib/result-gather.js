    ...
    async _resultsGatherInterval() {
        ...
        try {
            ...
            const jobsMerge = jobs.map(j => {
                ...
                if (g) {
                    g.graph.nodes.forEach(n => n.batchTasks = null)
                    g.graph.nodes.forEach(n => n.input = this._trimArray(n.input));
                    g.graph.nodes.forEach(n => n.output = this._trimArray(n.output));
                }
