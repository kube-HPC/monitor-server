...
//const jaegerApi = require('./jaeger/jager-api');
...
class ResultGather extends Events {
    ...
    _startInterval() {
        ...
        this.intervalId = setInterval(async () => {
            ...
            try {
                this._working = true;
                const nodeStatistics = await nodeStatisticsData.getLatestResult();
                ...
                algorithmsForDebug = algorithmsForDebug.map(a => (
                    {
                        ...a,
                        data: { ...a.data, path: `${prefix}/${a.key}/${suffix}` }
                    }))
                this.emit('result', {nodeStatistics, pipelinesStats, jobs: jobsMerge, discovery, algorithms, algorithmsForDebug, pipelines });
            }
            catch (error) {
                log.error(error);
            }
