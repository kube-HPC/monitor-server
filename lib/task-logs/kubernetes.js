...
class KubernetesLogs {
    ...
    async getLogs({ container, format, taskId, podName }) {
        ...
        const logs = await this._client.logs.get({ podName, containerName: container });
        return this._formalizeData(logs, taskId, formatMethod);
    }

    _formalizeData(logs, taskId, formatMethod) {
    ...
    _formatJson(str, task) {
        try {
            ...
            if (taskId && taskId === task && logComponent === 'Algorunner') {
                return line;
            }
        }
        catch (error) {
