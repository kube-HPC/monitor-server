...
class EsLogs {
    ...
    addComponentCriteria(nodeType) {
        const items = getSearchComponent(nodeType).map(sc => `meta.internal.component: "${sc}"`);
        if (items.length) {
            ...
            query: {
                bool: {
                    must: [
                        {
                            query_string: {
                                query: `meta.internal.taskId: "${taskId}" ${this.addComponentCriteria(nodeType)}`
                            }
                        }
                    ]
                }
