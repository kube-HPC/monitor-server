...
class NodesStatistics {
    ...
    async _statisticsInterval() {
        ...
        try {
            ...
            this._diskUsage = await this._getDiskUsage();
    ...
    getLatestResult() {
        return { nodeStatistics: this._currentResults, diskUsage: this._diskUsage };
    }
    ...
    async _getDiskUsage() {
        ...
        try {
            if (this._options.defaultStorage === 'fs') {
                ...
                const usagePercent = parseFloat((1 - (result.free / result.size)).toFixed(2)) * 100;
                return { usagePercent, ...result };
            }
        }
        catch (e) {
        }
