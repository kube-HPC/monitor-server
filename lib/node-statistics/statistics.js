...
class NodesStatistics {
    ...
    _buildAlgorithmResult(node, algorithms, metric, resourcePressure) {
        ...
        const nodeFree = ((node.total[metric] - node.requests[metric]) / (node.total[metric]));
        algorithmsData.push({
            name: 'free',
            amount: -1,
        ...
        algorithmsData.push({
            name: 'other',
            amount: otherAmount,
            size: (node.total[metric] * resourcePressure - (algorithmsData.reduce((sum, alg) => sum + alg.size, 0))).toFixed(1),
            percentage: 1 * resourcePressure - (algorithmsData.reduce((sum, alg) => sum + alg.percentage, 0))
        })
    ...
    async buildResultsForMetrics(metric) {
        ...
        const results = await Promise.all(taskExecutor.nodes.map(async node => {
            const algorithmsData = await this._buildAlgorithmResult(node, [...algorithms.algorithms, ...algorithms.algorithmsForDebug], metric, taskExecutor.resourcePressure[metric]);
            return {
                name: node.name,
                algorithmsData
            }
