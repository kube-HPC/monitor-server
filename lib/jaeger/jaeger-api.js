...
const path_ = 'http://40.69.222.75/jaeger/api/traces?service=api-server&tags={"jobId":"bool:e203fe49-a84e-4e7d-a27a-bc66fc127e21.bool"}'
...
const getTrace = async jobId => {
    ...
    try {
        const res = await axios.get(path_);
        const data = transformData(res.data.data[0]);
        console.log(data);
        return data;
    } catch (error) {
