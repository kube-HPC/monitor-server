...
const deleteAlgorithmFromStore = async algorithmName => {
...
const pauseExecution = jobId => {
    const path = `exec/pause`;
    const uri = `${baseUri}/${path}`;
    return request({
        method: "POST",
        uri,
...
const resumeExecution = jobId => {
    const path = `exec/resume`;
    const uri = `${baseUri}/${path}`;
    return request({
        method: "POST",
        uri,
...
const getReadme = async ({ name, type }) => {
