...
const updateStoredPipeline = ({pipeline}) => {
    const path = `store/pipelines`;
    const uri = `${baseUri}/${path}`;
    log.info(`Sending PUT request to ${uri} for updating stored pipeline=${pipeline.name} from simulator`, {
        component: 'api-server'
    });
...
const cronStart = ({pipelineName}) => {
    ...
    const uri = `${baseUri}/${path}`;
    log.info(`sending post request to ${uri} for cron schedule on pipeline=${pipelineName}`, {
        component: 'api-server'
    });
...
const cronStop = ({pipelineName}) => {
    ...
    const uri = `${baseUri}/${path}`;
    log.info(`sending post request (from simulator) with path=${path} for cron schedule on pipeline=${pipelineName}`, {
        component: 'api-server'
    });
